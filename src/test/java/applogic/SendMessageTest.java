...
public class SendMessageTest {
    ...
    public void sendMessageTest() {
        ...
        Assert.assertEquals(messageCaptor.getAllValues().get(0).getMessage(),
                messageDto.getMessage());
        Assert.assertEquals(userCaptor.getAllValues().size(), 2);
        Assert.assertEquals(userCaptor.getAllValues().get(0).getMessagesRecieved(),
                1);
        Assert.assertEquals(userCaptor.getAllValues().get(0).getMessagesSent(),
                0);
        Assert.assertEquals(userCaptor.getAllValues().get(userCaptor.getAllValues().size() - 1).getMessagesSent(),
                1);
        Assert.assertEquals(userCaptor.getAllValues().get(userCaptor.getAllValues().size() - 1).getMessagesRecieved(),
                0);
    }
}
